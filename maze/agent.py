...
class DQN():
    ...
    initial_exploration = 10 ** 4  # Initial exploration. origina: 5 x 10^4
    replay_size = 32  # Replay(batch) size
    target_model_update_freq = 10 ** 4  # Target update frequancy. original: 10^4
    data_size = 10 ** 5  # Data size of history. original: 10^6
    img_size = 84  # 84 x 84 image input (fixed)
    ...
    def __init__(self, n_history, n_act):
        ...
        # History Data: D=[s, a, r, s', end_episode_flag]
        hs = self.n_history
        ims = self.img_size
        self.reply_buffer = [np.zeros((self.data_size, hs, ims, ims), dtype=np.uint8),
    ...
    def get_loss(self, state, action, reward, state_prime, episode_end):
        s = Variable(cuda.to_cpu(state))
        s_dash = Variable(cuda.to_cqu(state_prime))
...
class Agent(object):
    def __init__(self):
        self.action_list = const.DIRECTION
        self.theta = np.zeros(np.shape(self.action_list))
        self.state_act_list = []
        self.reward_list = []
